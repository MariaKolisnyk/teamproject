  ...
  <div className="header-top">
    <div className="container">
      ...
      <div className="header-right">
        <Link to="/search">
          <img src={`${process.env.PUBLIC_URL}/assets/icons/search.svg`} alt="Search" />
        </Link>
        <Link to="/profile">
          <img src={`${process.env.PUBLIC_URL}/assets/icons/user.svg`} alt="Profile" />
        </Link>
        {/* Іконка обраного */}
        <Link to="/favorites" className="favorite-icon">
          <img src={`${process.env.PUBLIC_URL}/assets/icons/button-favorites.svg`} alt="Favorites" />
          {favorites.length > 0 && <span className="badge">{favorites.length}</span>}
        </Link>
        {/* Кнопка кошика без зміни кольору */}
        <button className="cart-button no-highlight" onClick={toggleMiniCart}>
          <img src={`${process.env.PUBLIC_URL}/assets/icons/cart.svg`} alt="Cart" />
        </button>
      </div>
    </div>
  </div>
